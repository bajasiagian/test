...
def get_summary_data(df):
    summary = df.copy()
    summary['date'] = pd.to_datetime(summary['date'], format='%d %b %Y', errors='coerce').dt.date
        ...
        for name in summary['employee_name'].unique():
            ...
            if len(temp) > 1:
                ...
                for i in range(len(temp)-1):
                    ...
                    time_between.append((datetime.strptime(temp.loc[i+1,'time_start'].strip(),'%H:%M:%S') - datetime.strptime(temp.loc[i,'time_start'].strip(),'%H:%M:%S')).total_seconds()/60)
...
def get_all_data(area_coverage,summary):
  ...
  for name in summary.employee_name.unique():
    ...
    if len(sum_) > 2: # At least 2 data points are needed for meaningful clustering
      wcss = []
      for i in range(2, min(9,len(sum_))): # Limit clusters to the number of data points if less than 9
        X= list(zip(sum_['latitude'],sum_['longitude']))
        kmeans = KMeans(n_clusters=i).fit(X)
        wcss.append(kmeans.inertia_)

      knee = KneeLocator(range(2, min(9,len(sum_))), wcss, curve="convex", direction="decreasing")
      ...
      for i in range(len(kmeans.cluster_centers_)):
        avail_[f'dist_center_{i}'] = avail_.apply(lambda x: get_distance_dealer(kmeans,i,x.latitude,x.longitude),axis=1)
    else:
